...
export function MoodPicker({ onChange, value }: { value?: MoodValue; onChange: (value: MoodValue) => void }) {
  ...
  return <View accessibilityRole="radiogroup" style={styles.options}>{options.map(option => <MoodOption key={option.value} illustration={<MoodIcon active={selected === option.value} mood={option.value} size={28} />} label={option.label} mood={option.value} selected={selected === option.value} onPress={() => onChange(option.value)} />)}</View>;
}
