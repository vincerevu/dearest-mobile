...
export function FlowPicker({ onChange, value }: { value?: FlowValue; onChange: (value: FlowValue) => void }) {
  return <View accessibilityRole="radiogroup" style={styles.options}>{([0, 1, 2, 3, 4] as const).map(level => <FlowOption key={level} label={labels[level]} level={level} selected={value === level} style={styles.option} onPress={() => onChange(level)} />)}</View>;
}
