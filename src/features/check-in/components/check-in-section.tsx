...
export function CheckInSection({ children, description, number, title }: CheckInSectionProps) {
  return (
    <View style={styles.section}>
      <View style={styles.header}><View style={styles.copy}><AppText variant="label">{title}</AppText>{description ? <AppText color="secondary" variant="label">{description}</AppText> : null}</View></View>
      {children}
    </View>
  );
}
