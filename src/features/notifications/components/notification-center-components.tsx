...
function NotificationRow({ item }: { item: NotificationItem }) { const color = item.unread ? colors.brand.primary : colors.text.muted; return <Card variant={item.unread ? 'soft' : 'default'}><View style={styles.row}>{item.icon === 'dovie' ? <DovieNavIcon color={color} size={30} /> : <MaterialCommunityIcons color={color} name={item.icon} size={24} />}<View style={styles.copy}><AppText variant="label">{item.title}</AppText><AppText color="secondary" variant="label">{item.message}</AppText></View>{item.unread ? <View style={styles.dot} /> : null}</View></Card>; }
const styles = StyleSheet.create({ copy: { flex: 1, gap: spacing.xs }, dot: { backgroundColor: colors.brand.primary, borderRadius: 4, height: 8, width: 8 }, row: { alignItems: 'center', flexDirection: 'row', gap: spacing.md }, stack: { gap: spacing.sm } });
