...
import { AppText, Button, ConfirmDialog, DateField, DisclosureRow, EmptyState, Input, SelectionRow, SettingsListRow, SettingsListSection, SwitchRow } from '@/components/ui';
...
export function ProfileManagementScreen() {
  ...
  return <Screen scroll><View style={styles.stack}><ScreenHeader title="Hồ sơ" /><DisclosureRow description={stageLabel} title="Hành trình của bạn" onPress={() => router.push('/life-stage')} /><DisclosureRow description="Quản lý chu kỳ, nhắc nhở, dữ liệu và các thiết lập ứng dụng." title="Tùy chọn cá nhân" onPress={() => router.push('/settings')} /></View></Screen>;
}
...
export function CycleSettingsManagementScreen() {
  ...
  return <Screen scroll><View style={styles.stack}><ScreenHeader title="Tùy chọn chu kỳ" onBack={() => router.back()} /><DisclosureRow description={cycle ? `${cycle.cycleLength} ngày · Hành kinh ${cycle.periodLength} ngày` : 'Chưa có dữ liệu chu kỳ.'} title="Thông tin chu kỳ" onPress={() => router.push('/cycle/edit')} /><SwitchRow description="Hiển thị kỳ kinh và cửa sổ thụ thai được ước tính trong tương lai. Dữ liệu bạn tự ghi nhận luôn hiển thị." label="Hiển thị dự đoán" value={predictions} onValueChange={setPredictions} /><View style={styles.infoSection}><AppText variant="label">LỊCH SỬ CHỈNH SỬA</AppText><AppText color="secondary">Lịch sử thay đổi chi tiết sẽ xuất hiện khi dữ liệu được đồng bộ với tài khoản của bạn.</AppText></View></View></Screen>;
}
...
export function WidgetsManagementScreen() {
  ...
  return <Screen scroll><View style={styles.stack}><ScreenHeader title="Widgets" onBack={() => router.back()} /><AppText color="secondary">Widget chỉ hiển thị bối cảnh ngắn, không hiển thị nhật ký, nội dung chat hay triệu chứng chi tiết.</AppText><SettingsListSection title="KIỂU WIDGET"><View accessibilityRole="radiogroup">{widgets.map(widget => <SelectionRow key={widget.id} description={widget.message} leading={<DovieAvatar decorative size={44} variant={widget.id} />} selected={selected === widget.id} title={widget.title} onPress={() => setSelected(widget.id)} />)}</View></SettingsListSection>
  </View></Screen>;
}
...
export function PrivacyManagementScreen() {
  ...
  return <Screen scroll><View style={styles.stack}><ScreenHeader title={translate('management.privacy')} onBack={() => router.back()} />
    <View style={styles.privacyHero}><Dovie decorative pose="privacy" size={88} /></View>
    <AppText color="secondary">{translate('management.privacy_description')}</AppText>
    <SettingsListSection title="CÁ NHÂN HÓA"><SwitchRow description={translate('management.ai_personalization_description')} label={translate('management.ai_personalization')} value={ai} onValueChange={setAi} /><SwitchRow description={translate('management.journal_ai_description')} label={translate('management.journal_ai')} value={journal} onValueChange={setJournal} /></SettingsListSection>
    <SettingsListSection title="CHIA SẺ"><SettingsListRow description="Quản lý dữ liệu bạn cho phép chia sẻ" label="Partner" onPress={() => router.push('/partner/sharing')} /></SettingsListSection>
    <SettingsListSection title="DỮ LIỆU TRÊN THIẾT BỊ">{deleted ? <EmptyState description={translate('management.deleted_local_data_description')} icon={<DovieMicro decorative size={72} state="empty" />} title={translate('management.deleted_local_data')} /> : <SettingsListRow description={translate('management.device_data_description')} label={translate('management.delete_local_data')} tone="danger" onPress={() => setConfirming(true)} />}</SettingsListSection>
...
const styles = StyleSheet.create({ accountAction: { paddingHorizontal: spacing.lg, paddingVertical: spacing.xs }, accountStack: { gap: spacing.xl }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }, avatarRow: { alignItems: 'center', flexDirection: 'row', gap: spacing.md }, flex: { flex: 1, gap: spacing.xs }, groupTitle: { fontSize: 12, letterSpacing: 0.6 }, infoSection: { gap: spacing.sm, paddingVertical: spacing.md }, pressed: { opacity: 0.68 }, privacyHero: { alignItems: 'center' }, profileAvatar: { alignItems: 'center', backgroundColor: colors.surface.soft, borderRadius: radius.pill, height: 48, justifyContent: 'center', width: 48 }, profileCard: { alignItems: 'center', backgroundColor: colors.surface.card, borderBottomColor: colors.border.soft, borderBottomWidth: 1, flexDirection: 'row', gap: spacing.md, minHeight: 72, paddingHorizontal: spacing.lg, paddingVertical: spacing.md }, regularity: { gap: spacing.sm }, rows: { backgroundColor: colors.surface.card, borderTopColor: colors.border.soft, borderTopWidth: 1 }, settingsGroup: { gap: spacing.sm }, settingsRow: { alignItems: 'center', borderBottomColor: colors.border.soft, borderBottomWidth: 1, flexDirection: 'row', gap: spacing.md, minHeight: 56, paddingHorizontal: spacing.lg }, settingsStack: { gap: spacing.xl }, stack: { gap: spacing.lg } });
