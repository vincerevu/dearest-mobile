...
export function FeaturedKnowledgeCard({ article, onPress }: { article: KnowledgeArticle; onPress: () => void }) {
  return <KnowledgeArticleCard article={article} onPress={onPress} />;
}
...
export function KnowledgeArticleCard({ article, onPress }: { article: KnowledgeArticle; onPress: () => void }) {
  return <Pressable accessibilityLabel={`Đọc bài ${article.title}`} accessibilityRole="button" style={({ pressed }) => [styles.articleRow, pressed && styles.pressed]} onPress={onPress}><View style={styles.articleCopy}><AppText color="brand" variant="label">{article.category}</AppText><AppText variant="headingMd">{article.title}</AppText>{article.excerpt ? <AppText color="secondary" numberOfLines={2}>{article.excerpt}</AppText> : null}{article.readTime ? <AppText color="muted" variant="label">{article.readTime}</AppText> : null}</View><AppText color="brand">›</AppText></Pressable>;
}

export function KnowledgeArticleGrid({ articles, onPressArticle }: { articles: KnowledgeArticle[]; onPressArticle: (article: KnowledgeArticle) => void }) {
  return <View style={styles.grid}>{articles.map(article => <KnowledgeArticleCard key={article.id} article={article} onPress={() => onPressArticle(article)} />)}</View>;
}
...
export function RelatedArticles({ articles, onPressArticle }: { articles: KnowledgeArticle[]; onPressArticle: (article: KnowledgeArticle) => void }) { return <KnowledgeArticleGrid articles={articles} onPressArticle={onPressArticle} />; }
