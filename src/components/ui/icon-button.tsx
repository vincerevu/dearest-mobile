...
type IconButtonProps = Omit<PressableProps, 'children' | 'style'> & { icon: ReactNode; style?: StyleProp<ViewStyle>; tone?: 'ghost' | 'default' | 'soft' | 'primary' };
...
export function NavigationIconButton({ accessibilityLabel, direction, onPress }: { accessibilityLabel: string; direction: 'back' | 'next'; onPress: () => void }) {
  ...
  return <IconButton accessibilityLabel={accessibilityLabel} hitSlop={8} icon={<MaterialCommunityIcons color={colors.navigation.icon} name={icon} size={26} />} tone="ghost" onPress={onPress} />;
}
...
const styles = StyleSheet.create({
  ...
  pressed: { backgroundColor: colors.navigation.pressed, opacity: 0.72 },
});
