...
export function SettingsListRow({ chevronTone = 'neutral', description, label, onPress, tone = 'default', trailing }: SettingsListRowProps) {
  ...
  const content = (
    <>
      <View style={styles.copy}>
        <AppText color={tone === 'danger' ? 'danger' : 'primary'} variant="label">{label}</AppText>
  ...
  if (onPress)
    return <Pressable accessibilityLabel={label} accessibilityRole="button" style={({ pressed }) => [styles.row, pressed && styles.pressed]} onPress={onPress}>{content}</Pressable>;
...
export function SettingsListSection({ children, title }: { title: string; children: ReactNode }) {
  return (
    <View style={styles.section}>
      <AppText color="muted" style={styles.sectionTitle} variant="label">{title}</AppText>
