...
export function ChecklistTaskRow({ completed, description, onToggle, title }: ChecklistTaskRowProps) {
  return (
    <Pressable accessibilityLabel={title} accessibilityRole="checkbox" accessibilityState={{ checked: completed }} style={({ pressed }) => [styles.row, completed && styles.completed, pressed && styles.pressed]} onPress={onToggle}>
      <MaterialCommunityIcons color={completed ? colors.brand.primary : colors.text.muted} name={completed ? 'check-circle' : 'checkbox-blank-circle-outline'} size={26} />
      <View style={styles.copy}>
        <AppText variant="label">{title}</AppText>
        <AppText color="secondary" variant="label">{completed ? 'Đã hoàn thành' : description}</AppText>
      </View>
    </Pressable>
  );
}
